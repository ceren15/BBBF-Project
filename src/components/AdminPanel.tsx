...
const SidebarMenu = ({ title, icon: Icon, menuId, expandedMenus, toggleMenu, children }: any) => (
  <div>
    <div onClick={() => toggleMenu(menuId)} className="px-4 py-2 flex items-center justify-between cursor-pointer hover:bg-gray-100">
    ...
    {expandedMenus[menuId] && <div className="pl-5 bg-gray-50">{children}</div>}
  </div>
);
...
export default function AdminPanel() {
  ...
  return (
    <div className="flex h-screen bg-white">
      {sidebarOpen && (
        <div className="w-64 bg-gray-50 shadow-md relative">
          <div className="p-4">
          ...
          <div className="py-2">
            <div className="px-4 py-2 text-sm text-gray-600">Platform</div>
            <div className="mt-1">
              <SidebarMenu menuId="panel" title="Panel" icon={HiOutlineSquares2X2} expandedMenus={expandedMenus} toggleMenu={toggleMenu}>
                <div className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Genel Bakış</div>
              </SidebarMenu>
              <SidebarMenu menuId="staj-yonetimi" title="Staj Yönetimi" icon={VscBook} expandedMenus={expandedMenus} toggleMenu={toggleMenu}>
                <div className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Staj Dönemleri</div>
                <div onClick={() => router.push('/applications')} className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Başvurular</div>
                <div className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Resmi Tatiller</div>
              </SidebarMenu>
              <SidebarMenu menuId="ogrenci-islemleri" title="Öğrenci İşlemleri" icon={GoPeople} expandedMenus={expandedMenus} toggleMenu={toggleMenu}>
                <div className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Öğrenci Listesi</div>
                <div onClick={() => router.push('/student-activities')} className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Öğrenci Aktiviteleri</div>
              </SidebarMenu>
              <SidebarMenu menuId="sistem-yonetimi" title="Sistem Yönetimi" icon={VscSettingsGear} expandedMenus={expandedMenus} toggleMenu={toggleMenu}>
                <div className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">Yönetici Listesi</div>
              </SidebarMenu>
            </div>
          </div>

          <div className="absolute bottom-0 w-64 py-4 px-4">
      ...
      <div className="flex-1 p-8 overflow-auto">
        ...
        <div className="grid grid-cols-10 gap-6">
          <div className="col-span-5 bg-white p-6 rounded shadow-sm border border-gray-200">
            <h2 className="font-medium mb-4">Başvuru İstatistikleri</h2>
            <div className="flex flex-col items-center justify-center h-32">
              <FileTextIcon />
              <p className="text-sm text-gray-500 mt-2">Henüz başvuru bulunmamaktadır.</p>
            </div>
          </div>
          ...
          <div className="col-span-5 flex flex-col gap-6">
            ...
            <div className="bg-white p-6 rounded shadow-sm border border-gray-200">
              ...
              <div className="flex flex-col items-center justify-center h-20">
                <UsersIcon />
                <p className="text-sm text-gray-500 mt-2">Henüz öğrenci kaydı bulunmamaktadır.</p>
              </div>
            </div>
          </div>

          <div className="col-span-10 bg-white p-6 rounded shadow-sm border border-gray-200">
            <h2 className="font-medium mb-4">Öğretmen Ataması</h2>
            <div className="flex flex-col items-center justify-center h-32">
              <FileTextIcon />
              <p className="text-sm text-gray-500 mt-2">Henüz atama bulunmamaktadır.</p>
            </div>
          </div>
        </div>
      </div>
